...
const Page: FaustPage<{}> = () => {
	...
	const [avatarImage, setAvatarImage] = useState<ImageState>({
		id: '',
		sourceUrl: '',
		altText: '',
	})
  ...
  `),
	...
	const [mutationGetDeleteAccountNonce, getDeleteAccountNonceResult] =
		useMutation(
			gql(`mutation ProfilePageMutationGetDeleteAccountNonce($user_email: String!) {
            ncmazFaustCreateDeleteAccountNonce(input: {user_email: $user_email}) {
              errors
              nonce
              success
            }
	...
	const [mutationDeleteAccountByNonce, deleteAccountByNonceResult] =
		useMutation(
			gql(`mutation ProfilePageMutationDeleteAccountByNonce($user_email: String!, $nonce: String!) {
        ncmazFaustDeleteAccountByNonce(
          input: {nonce:  $nonce, user_email: $user_email}
        ) {
          success
          errors
        }
	...
	const DELETE_NONCE =
		getDeleteAccountNonceResult.data?.ncmazFaustCreateDeleteAccountNonce
			?.nonce || ''
	const USER_EMAIL = viewer?.email || ''
	const DELETE_TEXT_CONFIRMED = confirmDeleteText === 'Delete my account'
	const LOADING =
		updateViewerProfileResult.loading || deleteAccountByNonceResult.loading
	...
	useEffect(() => {
		if (!USER_EMAIL || IS_CHISNGHIAX_DEMO_SITE) {
			return
		}
		mutationGetDeleteAccountNonce({
			variables: {
	...
	const handleDeleteAccount = () => {
		if (LOADING) {
			toast.error('Wait a moment, data is loading!')
			return
		}
		if (!DELETE_TEXT_CONFIRMED) {
			toast.error('Please type "Delete my account" to confirm!')
			return
		}

		if (!isAuthenticated) {
			toast.error('You must login to delete account!')
			router.push('/login')
			return
		}
		...
		if (!USER_EMAIL) {
			toast.error('Wait a moment, data is being prepared!')
			return
		}
			...
			<div className="flow-root">
				<div className="NcmazAccountPage-deleteAccount space-y-5 sm:space-y-6 md:sm:space-y-7">
					...
					<div>
						...
						/>
						<>
							{deleteAccountByNonceResult.error && (
								<Error error={deleteAccountByNonceResult?.error?.message} />
							)}
						</>
					</div>
				</div>
				...
				<div className="NcmazAccountPage-GeneralForm space-y-5 sm:space-y-6 md:sm:space-y-7">
					...
					<div className="SocialsProfileForm__fieldsWrap grid grid-cols-1 gap-5 sm:grid-cols-2">
						...
						<div className="SocialsProfileForm__Buymeacoffe">
							...
							<span className="mb-2 mt-1 block text-xs text-neutral-500 dark:text-neutral-400">
	...
	const renderMainContent = () => {
		return (
			<div className={currentTab === 'socials' ? 'max-w-4xl' : 'max-w-2xl'}>
				{renderContent()}

				{isAuthenticated && USER_EMAIL && (
					<div className="mt-8">
						{/* ---- */}
						{updateViewerProfileResult.error && (
							<Alert type="error" containerClassName="mb-5">
								{decode(updateViewerProfileResult.error?.message || '')}
							</Alert>
						)}
						{/* ---- */}
		...
		<>
			<DashboardLayout>
				<div className="px-4 sm:px-6 lg:px-8">
					{USER_EMAIL ? renderMainContent() : <CircleLoading />}
				</div>
			</DashboardLayout>
		</>
	)

...
export interface Props {
	...
	post: FragmentTypePostFullFields | FragmentType<typeof NC_POST_CARD_FRAGMENT>
	isSingle?: boolean
}
...
const PostActionDropdown: FC<Props> = ({
	...
	useEffect(() => {
		if (loading) {
			toast.loading('Обновление поста...')
		}
	}, [loading])
	...
	if (viewer?.capabilities?.includes('edit_others_posts')) {
	...
	const hanldeClickDropDown = (item: TPostActionitem) => {
		...
		if (loading) {
			toast.loading('Обновление поста, пожалуйста подождите!')
			return
		}
		//
